...
//#define bt_nvram_log(M, ...) custom_log("BT NVRAM", M, ##__VA_ARGS__)
#define bt_nvram_log(M, ...)
...
static mico_bool_t mico_bt_nvram_access_valid_entry(mico_bt_nvram_access_entry_t *p_entry)
{
    ...
    bt_nvram_log ("mico_bt_nvram_access_valid_entry  BDA: %08x%04x, key length: %d",
                      (p_entry->bd_addr[0]<<24)+(p_entry->bd_addr[1]<<16)+(p_entry->bd_addr[2]<<8)+p_entry->bd_addr[3],
                      (p_entry->bd_addr[4]<<8)+p_entry->bd_addr[5], p_entry->length);

    if(memcmp(p_entry->bd_addr,bd_addr_any,sizeof(bd_addr_any)) && memcmp(p_entry->bd_addr,null_addr,sizeof(mico_bt_device_address_t)) &&
...
void mico_bt_nvram_access_init()
{
    ...
    /*
          For application based on WICED SDK, application needs to know the information of
...
static mico_bool_t mico_bt_nvram_access_find_offset(mico_bt_device_address_t key_bdaddr, int start_offset , int *p_offset)
{
     ...
     {
         /* read bd_addr key*/
      //   mico_dct_read_lock( (void**) &p_entry, MICO_FALSE, DCT_APP_SECTION, of_dct, MICO_BT_DCT_ADDR_FIELD+ MICO_BT_DCT_LENGTH_FIELD);

		//fixed Only one bond device could be accessed, p_entry->length may be used without valid value  modified by kevin
...

{
     ...
     bt_nvram_log ("mico_bt_nvram_access_save_bonded_device_key  BDA: %08x%04x, key length: %d",
                       ((bd_addr)[0]<<24)+((bd_addr)[1]<<16)+((bd_addr)[2]<<8)+(bd_addr)[3],
                       ((bd_addr)[4]<<8)+(bd_addr)[5], key_len );
    ...
    //          mico_dct_write(&addr_type, DCT_APP_SECTION, of_key + MICO_BT_DCT_ADDR_FIELD + MICO_BT_DCT_LENGTH_FIELD , sizeof(mico_bt_ble_address_type_t));
    //          mico_dct_write(&device_type, DCT_APP_SECTION, of_key + MICO_BT_DCT_ADDR_FIELD + MICO_BT_DCT_LENGTH_FIELD + MICO_BT_DCT_ADDR_TYPE , MICO_BT_DCT_DEVICE_TYPE);
			 // mico_dct_write(&key_len, DCT_APP_SECTION, of_key + MICO_BT_DCT_ADDR_FIELD , sizeof(uint16_t));
         ...
         }else
         {
             ...
             {
                bt_nvram_log( "Same key, write ignore..." );
             }
             //mico_dct_write(p_keyblobs, DCT_APP_SECTION, of_key + MICO_BT_DCT_ENTRY_HDR_LENGTH, key_len );
         }
...
mico_bt_result_t mico_bt_nvram_access_load_bonded_device_keys(mico_bt_device_address_t bd_addr, mico_bt_nvram_access_entry_t *p_key_entry,uint8_t entry_max_length)
{
    ...
    if(mico_bt_nvram_access_find_offset(bd_addr, dct_cb.start_offset, &of_key))
    {
        ...
        /* read key entry*/
        mico_system_para_read( (void**) &p_entry, PARA_BT_DATA_SECTION, of_key, MICO_BT_DCT_ENTRY_HDR_LENGTH);
        //mico_dct_read_lock( (void**) &p_entry, MICO_FALSE, DCT_APP_SECTION, of_key, MICO_BT_DCT_ENTRY_HDR_LENGTH);
        ...
        //mico_dct_read_lock( (void**) &p_keyblobs, MICO_FALSE, DCT_APP_SECTION,
        //        of_key + MICO_BT_DCT_ENTRY_HDR_LENGTH,
        //        p_entry->length);
...
mico_bt_result_t mico_bt_nvram_access_delete_bonded_device(mico_bt_device_address_t bd_addr)
{
    ...
    //mico_dct_write(invalidate,DCT_APP_SECTION,dct_cb.end_offset, del_length);  /* set as 0xff */
...
mico_bt_result_t mico_bt_nvram_access_load_local_identity_keys(mico_bt_local_identity_keys_t *p_lkeys)
{
    ...
    mico_system_para_read( (void**) &p_local_key, PARA_BT_DATA_SECTION, dct_cb.start_offset - sizeof(mico_bt_local_identity_keys_t), BTM_SECURITY_LOCAL_KEY_DATA_LEN );

    //result = mico_dct_read_lock( (void**) &p_local_key, MICO_FALSE, DCT_APP_SECTION, dct_cb.start_offset - MICO_BT_DCT_LOCAL_KEY_OFFSET,
    ...
    //mico_dct_read_unlock((void*)p_local_key, MICO_FALSE);
...
mico_bt_result_t mico_bt_nvram_access_save_local_identity_keys(mico_bt_local_identity_keys_t *p_lkeys)
{
    ...
    mico_system_para_write(p_lkeys, PARA_BT_DATA_SECTION, dct_cb.start_offset - sizeof(mico_bt_local_identity_keys_t), sizeof(mico_bt_local_identity_keys_t) );
    //mico_dct_write((void*) p_lkeys,
    //        DCT_APP_SECTION, dct_cb.start_offset - MICO_BT_DCT_LOCAL_KEY_OFFSET, sizeof(mico_bt_local_identity_keys_t));
...
BOOLEAN mico_bt_nvram_access_key_storage_available(mico_bt_device_address_t bd_addr, int req_size)
{
     ...
     APPL_TRACE_DEBUG3("mico_bt_nvram_access_key_storage_available :%d  %d   %d",dct_cb.dct_size,
                         dct_cb.end_offset,dct_cb.start_offset );
     ...
     APPL_TRACE_DEBUG3("mico_bt_callout_key_storage_available : found %d, mem_av:%d, req_size:%d", found, mem_available,
                 req_size);
...
mico_bt_result_t mico_bt_nvram_access_enum_bonded_device_keys(int8_t *p_index, mico_bt_nvram_access_entry_t * p_key_entry, uint8_t entry_max_length)
{
    ...
    mico_system_para_read((void**) &p_entry, PARA_BT_DATA_SECTION, dct_cb.cur_offset, MICO_BT_DCT_ENTRY_HDR_LENGTH );
    //mico_dct_read_lock( (void**) &p_entry, MICO_FALSE, DCT_APP_SECTION, dct_cb.cur_offset, MICO_BT_DCT_ENTRY_HDR_LENGTH);
    if(!mico_bt_nvram_access_valid_entry(p_entry))
    {
        dct_cb.cur_offset = dct_cb.end_offset;
        mico_system_para_read_release( (void*) p_entry );
    ...
    //mico_dct_read_lock( (void**) &p_key_blobs, MICO_FALSE, DCT_APP_SECTION, dct_cb.cur_offset + MICO_BT_DCT_ENTRY_HDR_LENGTH, p_entry->length);
    ...
    *p_index = ++dct_cb.index_enum;
    ...
    //mico_dct_read_unlock( (void*) p_key_blobs, MICO_FALSE);
    //mico_dct_read_unlock( (void*) p_entry, MICO_FALSE);
