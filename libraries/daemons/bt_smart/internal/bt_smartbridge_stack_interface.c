...
static void smartbridge_scan_result_callback( mico_bt_ble_scan_results_t *p_scan_result, uint8_t *p_adv_data )
{
    if( p_scan_result )
    {
        ...
        memcpy( advertising_report.eir_data, p_adv_data, 31);
        advertising_report.eir_data_length = 0;

        bt_smartbridge_log( "address:%x %x %x %x %x %x rssi:%d bdaddrtype:%d event_type:%x",
        ...
        if( p_scan_result->ble_evt_type == BT_SMART_SCAN_RESPONSE_EVENT )
        {
            ...
            if( app_scan_report_callback != NULL )
            {
                ...
                app_scan_report_callback( &current_scan_result->last_scan_response_received );
            }
        }
        else
        {
            ...
            {
                bt_smartbridge_log( "advertising callback reported" );
                app_scan_report_callback( &current_scan_result->last_advertising_event_received );
            }
        }
    }
    ...
    else
    {
        ...
        {
            mico_rtos_send_asynchronous_event( MICO_NETWORKING_WORKER_THREAD, app_scan_complete_callback, NULL );
        }
    }
}
...
mico_bool_t smartbridge_bt_interface_add_device_to_whitelist( const mico_bt_device_address_t* device_address, mico_bt_smart_address_type_t address_type )
{
    ...
    return mico_bt_ble_update_advertising_white_list( MICO_TRUE, *(mico_bt_device_address_t* )device_address );
}
...
{
    UNUSED_PARAMETER(address_type);
    return mico_bt_ble_update_advertising_white_list( MICO_FALSE, *(mico_bt_device_address_t* )device_address );
}

OSStatus smartbridge_bt_interface_get_whitelist_size( uint32_t *size )
{
  //TODO: 
    //*size = mico_bt_ble_get_white_list_size();
    return MICO_BT_UNSUPPORTED;
}
...
{
  //TODO: 
    //return mico_bt_ble_clear_white_list();
    return MICO_BT_UNSUPPORTED;
}
